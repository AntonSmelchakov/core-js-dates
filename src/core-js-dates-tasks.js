...
function getNextFriday(date) {
  ...
  const mult = day < 5 ? 5 - day : 5 + (7 - day);
  const currentTime = dateDate.getTime();
  return new Date(currentTime + 86400000 * mult);
}
...
function getCountDaysInMonth(month, year) {
  ...
  if (month === 12) {
    ...
    date2 = new Date(year + 1, 0);
  }
  const res = date2.getTime() - date.getTime();
  return Math.ceil(res / 86400000);
}
...
function formatDate(date) {
  const myDate = new Date(date);
  const [year, month, day, hour, minutes, seconds, pmAm] = [
    myDate.getUTCFullYear(),
    myDate.getUTCMonth() + 1,
    myDate.getUTCDate(),
    myDate.getUTCHours() <= 12
      ? myDate.getUTCHours()
      : myDate.getUTCHours() - 12,
    `${myDate.getUTCMinutes()}`.padStart(2, '0'),
    `${myDate.getUTCSeconds()}`.padStart(2, '0'),
    myDate.getUTCHours() >= 12 ? 'PM' : 'AM',
  ];
  return `${month}/${day}/${year}, ${hour}:${minutes}:${seconds} ${pmAm}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const dateEnd = new Date(year, month);
  while (date < dateEnd) {
...
function getWeekNumberByDate(date) {
  ...
  const start = new Date(Date.UTC(year, 0));
  const firstDay = start.getUTCDay();
  let offset;
  if (firstDay === 0) offset = -1;
  else if (firstDay <= 4) offset = firstDay - 1;
  else if (firstDay > 4) offset = firstDay - 8;
  const offsetValue = offset * 86400000;
  const value = myDate - start + offsetValue;
  return Math.floor(value / 604800000) + 1;
}
...
function getQuarter(date) {
  ...
  let result;
  for (let n = 1; n * 3 <= 12; n += 1) {
    if (num >= n * 3 - 2 && num <= n * 3) result = n;
  }
  return result;
}
